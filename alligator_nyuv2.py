...
from models.alligator_downstream import AlligatorDownstreamMonocularEncoder, checkpoint_args_from_ckpt
...
def get_linear_head(model, num_classes=1):
    head = nn.Sequential(
        nn.Linear(model.head.dpt.layer_dims[-1], 512),
        nn.ReLU(),
        nn.Linear(512, num_classes)
    )
    return head

def load_model(freeze=True, decoder=False, head_type="DPT"):
    ...
    if head_type=="DPT":
        ...
        print('head: Linear()')
        head = get_linear_head(model)
    else:
        head=None

    model = AlligatorDownstreamMonocularEncoder(head, **ckpt_args.croco_args)
    msg = model.load_state_dict(ckpt['model'], strict=False, weights_only=False)
