...
class AligatorNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.imu_enc_embed_dim = imu_enc_embed_dim
        self.imu_enc_depth = imu_enc_depth
        self.imu_enc_num_heads = imu_enc_num_heads

        # imu encoder mask generator
        self._set_imu_mask_generator(imu_length, imu_mask_ratio)
        # transformer for imu encoder
        self.imu_encoder = TransformerEncoderWithRoPE(
                input_dim=imu_features,
                d_model=imu_enc_embed_dim,
                nhead=imu_enc_num_heads,
                num_layers=imu_enc_depth,
                mlp_ratio=4.,
                dropout=0.1)
    ...
    def forward(self, img1, img2, imu, imu_length):
        ...
        # decoder 
        dec_img_feat, dec_imu_feat = self._decoder(feat1, pos1, mask1, feat2, pos2, imu, imu_mask, imu_attn_mask)
        # prediction head 
        out_img = self.prediction_head(dec_img_feat)
        out_imu = self.imu_prediction_head(dec_imu_feat)
        # get target
        target = self.patchify(img1)
        return out_img, out_imu, mask1, target
